...
// 【用户浏览时长统计】
// ==============================
const visitStartTime = new Date(); // 记录进入时间

// 用户离开页面时（关闭/刷新/跳转）计算时长
window.addEventListener('beforeunload', function() {
    const endTime = new Date();
    const stayTimeMs = endTime - visitStartTime;
    const stayTimeSecond = Math.floor(stayTimeMs / 1000); // 浏览秒数

    // 数据发给后台
    const logData = {
        url: window.location.href,
        enterTime: visitStartTime.toLocaleString(),
        leaveTime: endTime.toLocaleString(),
        staySeconds: stayTimeSecond,
        stayTime: stayTimeSecond + " 秒"
    };

    // 打印到控制台 → 你在浏览器后台就能看到！
    console.log("【用户浏览记录】", logData);
...
setInterval(() => {
    const now = new Date();
    const seconds = Math.floor((now - visitStartTime) / 1000);
    const info = `当前已浏览：${seconds} 秒`;
    
    // 把信息显示在页面底部
    let status = document.getElementById('visit-status');
    if (!status) {
        status = document.createElement('div');
        status.id = 'visit-status';
        status.style.cssText = 'text-align:center; padding:8px; color:#888; font-size:13px;';
